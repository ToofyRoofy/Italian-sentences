// ============================================================================
// 📌 قاعدة أساسية لأي إضافة محتوى جديد (كلمة / جملة درس / سطر محادثة / فقرة
// استماع أو قراءة) — اتكتبت بعد ما اتكشف إن كتير من الكلمات في كل أنحاء
// التطبيق كانت مربوطة بموضوع نحوي هنا بس مش فعليًا موجودة جوّاه (فيفتح بوب أب
// فاضي)، أو مش مربوطة خالص. أي AI هيضيف محتوى جديد لازم يتبع الخطوات دي لكل
// عنصر جوه أي مصفوفة `words: [...]` (في WORDS، lesson_*.js، scenes.js،
// LISTENING_PASSAGES/LIBRO_PASSAGES/NUOVO_PASSAGES في app.js):
//
// ١) اختار موضوع نحوي مناسب من GRAMMAR تحت (أو زوّد بلوك جديد في موضوع موجود،
//    أو افتح موضوع جديد لو محتاج) واكتب الـ id بتاعه في `grammarId` بتاع
//    الكلمة صراحةً. من غير `grammarId`، الكود بيحاول يستنتج الموضوع تلقائي عن
//    طريق findGrammarTopicId(w.it) اللي بيدوّر بس في triggers[] بتاعت كل
//    موضوع — وده بيتطابق مع الكلمة الأساسية بس (مش تصريفات/جموع/صيغ)، فمتعتمدش
//    عليه لوحده.
//
// ٢) الأهم: تأكد إن الموضوع بالفعل فيه عنصر يخص شكل الكلمة دي بالظبط، مش بس
//    "مرتبط مفهوميًا". البوب أب بيدوّر جوه blocks الموضوع (تطابق تام → tokens
//    العبارة → نفس الجذر → نص داخل أي مثال/صف جدول) وبيضيء (flash) أول حاجة
//    تطابق. يعني لازم شكل الكلمة (أو جذرها، أو العبارة كاملة لو أكتر من كلمة)
//    يكون موجود فعليًا في واحد من دول:
//      - item.it أو item.formAliases[]  (لصيغ زي negozi لـ Negozio: زوّد
//        "negozi" في formAliases بتاع الـ item بتاع Negozio)
//      - usage.form أو usage.formAliases[] أو usage.examples[].form
//      - أول عمود (أو أي عمود) في صف جدول (rows)
//      - نص examples[].it بتاع أي item/usage (مفيد جدًا للعبارات المركبة زي
//        "fare la spesa" — خلي العبارة كاملة موجودة كمثال، مش بس كلمة منها)
//    لو مفيش حاجة من دول، هيفضل يفتح "مفيش شرح مخصوص للكلمة دي" — مش error، بس
//    مش مفيد. تأكد بعد الإضافة بفتح الموضوع ودوسة على الكلمة، وشوف هل فيه
//    عنصر اتضوّى (.flash) جوه البوب أب.
//
// ٣) صنّف كل كلمة بحقل `type` بنفس التصنيفات المستخدمة في buildWordInfoHtml
//    (app.js): verbo / nome_unit / aggettivo / pronome / avverbio /
//    congiunzione / sintagma_prep / omesso (فعل مساعد بيتحذف زي "ho" في
//    "ho visto") / altro. verbo محتاج كمان verbInfinitive ومطابقة في verbs.js.
//
// ٤) أي item/usage جديد بتضيفه هنا لازم يكون عنده examples: [{it,ar}] واحد
//    على الأقل — من غيره الكلمة بتتربط "بمكان فاضي".
//
// ─────────────────────────────────────────────────────────────────────────
// 🎨 خاص بحروف الجر (di/a/da/in/con/su/per/tra‑fra): كل واحد فيهم مقسّم في
// preposition_profiles.js (PREPOSITION_USAGE_PROFILES) لاستخدامات محددة —
// كل استخدام له لون مميز (color) وblock من نوع "usage" هنا بنفس اللون
// بالظبط (شوف prep_di/prep_a/prep_da/prep_in/prep_con/prep_su/prep_per/
// prep_tra_fra — نفس نمط الـusage+color مستخدم كمان في partitivi/colori/
// numeri لو احتجته هناك). لما تضيف جملة فيها حرف جر (بسيط زي "di" أو
// مندمج زي "dal/nel/sul..."):
//   ١) حدد الاستخدام بالظبط (مش بس حرف الجر عمومًا) من الاستخدامات
//      الموجودة فعليًا — مثلاً "الشنطة دي بتاعة مريم" = di_possesso،
//      "أنا جاي من روما" = di_origine، مش مجرد "di" كحرف جر عام.
//   ٢) في words[] بتاعة الجملة، عنصر حرف الجر نفسه يدّي: type:"preposizione"،
//      grammarId=id الموضوع (زي "prep_di")، وcolor=نفس اللون المكتوب في
//      usage block بتاع الاستخدام ده بالظبط (مش لون عام للموضوع كله) —
//      عشان الدوسة على الكلمة في السياق ده تفتح شرح الاستخدام الصحيح
//      بالظبط، مش أول حاجة تطابق بالصدفة (شوف focusColor في openGrammarModal
//      بتاع app.js).
//   ٣) ضيف الجملة نفسها (it/ar/form + words[] كاملة، بنفس شكل الأمثلة
//      الموجودة فعلاً جوه كل usage block هنا) كعنصر جديد في examples[]
//      بتاعة الـusage block ده تحديدًا — إضافة جنب الأمثلة الموجودة، مش
//      استبدال للمثال الوحيد اللي فيه.
// ده منفصل تمامًا عن sentence.prepositionFocus/prepositionExposure اللي في
// lesson_manifest.js — دي آلية تعرّض دورية متعمدة بتلف على كل الاستخدامات
// بالتبادل لكل جملة بغض النظر عن محتواها (لمراجعة متباعدة)، مش مفروض
// تتربط بمحتوى الجملة الفعلي، وسيبها زي ما هي.
// ─────────────────────────────────────────────────────────────────────────
//
// ⚠️ ملحوظة عن الـfallback الموجود: أي كلمة (حتى المتربطتش بحاجة خالص) دلوقتي
// بتفتح بوب أب بيورّي معناها المحفوظ (w.ar/w.note) على الأقل — شوف
// openWordFamily في app.js. ده شبكة أمان عشان الدوسة ما تفضلش من غير رد فعل،
// مش بديل عن الربط الصح فوق. كلمة بتعتمد على الـfallback ده يبقى لسه ناقصها
// الخطوات ١-٤ (أو خطوات حروف الجر فوق لو الكلمة حرف جر).
// ============================================================================
const GRAMMAR = [
  {
    "id": "interrogativi",
  ...
  },
  {
    ...
    "blocks": [
      ...
      {"type":"item","it":"Giovanile","ar":"شبابي","note":"صفة من giovane (شاب)","examples":[{"it":"Ha uno stile giovanile","ar":"عندها ستايل شبابي"}],"formAliases":["giovanili"]},
      {"type":"item","it":"Caro","ar":"غالي","note":"عكسها Economico — تحذير: Caro قبل الاسم ممكن يبقى معناها 'عزيزي'","examples":[{"it":"È un po' caro","ar":"غالي شوية"}],"formAliases":["cari"]},
      {"type":"item","it":"Pesante","ar":"تقيل","note":"جمعها Pesanti — عكسها Leggero (خفيف)","examples":[{"it":"I maglioni pesanti","ar":"البلوفرات التقيلة"}],"formAliases":["pesanti"]},
      {"type":"item","it":"Stretto","ar":"ضيق","note":"جمعها Stretti، مؤنثها Stretta/Strette — عكسها Largo","examples":[{"it":"I jeans stretti","ar":"الجينز الضيق"}],"formAliases":["stretti","stretta","strette"]},
      {"type":"item","it":"Corto","ar":"قصير (طول الحاجة)","note":"جمعها Corti، مؤنثها Corta/Corte — عكسها Lungo","examples":[{"it":"Le gonne corte","ar":"الجيبان القصيرة"}],"formAliases":["corti","corta","corte"]},
      {"type":"item","it":"Largo","ar":"واسع","note":"جمعها Larghi، مؤنثها Larga/Larghe — عكسها Stretto","examples":[{"it":"I pantaloni larghi","ar":"البناطيل الواسعة"}],"formAliases":["larghi","larga","larghe"]},
      {"type":"item","it":"Prossimo","ar":"الجاي / القادم","note":"جمعها Prossimi، مؤنثها Prossima/Prossime","examples":[{"it":"La prossima stagione","ar":"الموسم الجاي"}],"formAliases":["prossima","prossimi","prossime"]},
      {"type":"item","it":"Bello","ar":"جميل","note":null,"examples":null,"formAliases":["bella"]},
      {"type":"item","it":"Famoso","ar":"مشهور","note":null,"examples":null},
      {"type":"item","it":"Grande","ar":"كبير","note":null,"examples":null},
      {"type":"item","it":"Interessante","ar":"ممتع / مثير للاهتمام","note":null,"examples":null},
      {"type":"item","it":"Nuovo","ar":"جديد","note":null,"examples":null,"formAliases":["nuova"]},
      {"type":"item","it":"Americano","ar":"أمريكي","note":null,"examples":null},
      {"type":"item","it":"Bravo","ar":"شاطر / كويس","note":null,"examples":null},
      {"type":"item","it":"Francese","ar":"فرنسي","note":null,"examples":null},
      {"type":"item","it":"Giovane","ar":"شاب","note":null,"examples":null},
      {"type":"item","it":"Importante","ar":"مهم","note":null,"examples":null},
      {"type":"item","it":"Italiano","ar":"إيطالي (صفة)","note":"مؤنثها Italiana — غير Italiano الاسم (اللغة)","examples":null,"formAliases":["italiana"]},
      {"type":"item","it":"Moderno","ar":"عصري / حديث","note":null,"examples":null},
      {"type":"item","it":"Piccolo","ar":"صغير","note":null,"examples":null,"formAliases":["piccola"]},
      {"type":"item","it":"Popolare","ar":"شعبي / مشهور","note":null,"examples":null},
      {"type":"item","it":"Simpatico","ar":"لطيف","note":null,"examples":null,"formAliases":["simpatica"]},
      {"type":"item","it":"Spagnolo","ar":"إسباني","note":null,"examples":null},
      {"type":"item","it":"Vecchio","ar":"قديم / عجوز","note":null,"examples":null},
      {"type":"item","it":"Straniero","ar":"أجنبي","note":null,"examples":null,"formAliases":["straniera"]},
      {"type":"item","it":"Rosso","ar":"أحمر","note":"مؤنثها Rossa","examples":null,"formAliases":["rossa"]}
    ]
  },
  {
    "id": "lex_nomi",
    ...
    "blocks": [
      ...
      {"type":"item","it":"Ufficio","ar":"مكتب","note":null,"examples":null},
      {"type":"item","it":"Taglia","ar":"مقاس","note":null,"examples":null},
      {"type":"item","it":"Modello","ar":"موديل","note":null,"examples":null},
      {"type":"item","it":"Regalo","ar":"هدية","note":null,"examples":null},
      {"type":"item","it":"Signora","ar":"يا مدام / حضرتك","note":"صيغة مخاطبة مؤدبة للستات","examples":null},
      {"type":"item","it":"Scontrino","ar":"إيصال / فاتورة","note":null,"examples":null},
      {"type":"item","it":"Ginnastica","ar":"جمباز / رياضة","note":"في تعبير ثابت: scarpe da ginnastica = جزمة رياضي (سنيكرز)","examples":[{"it":"Le scarpe da ginnastica","ar":"جزم الرياضة"}]},
      {"type":"item","it":"Stagione","ar":"موسم / فصل السنة","note":null,"examples":[{"it":"Per la prossima stagione","ar":"للموسم الجاي"}],"formAliases":["stagioni"]},
      {"type":"item","it":"Classe","ar":"فصل / حجرة الدراسة","note":null,"examples":null},
      {"type":"item","it":"Studente","ar":"طالب","note":"مؤنثها Studentessa","examples":null},
      {"type":"item","it":"Albergo","ar":"فندق","note":null,"examples":null},
      {"type":"item","it":"Albero","ar":"شجرة","note":null,"examples":null},
      {"type":"item","it":"Architetto","ar":"مهندس معماري","note":null,"examples":null},
      {"type":"item","it":"Astuccio","ar":"مقلمة","note":null,"examples":null},
      {"type":"item","it":"Aula","ar":"قاعة / فصل","note":null,"examples":null},
      {"type":"item","it":"Autobus","ar":"أوتوبيس","note":null,"examples":null},
      {"type":"item","it":"Blog","ar":"بلوج","note":null,"examples":null},
      {"type":"item","it":"Cane","ar":"كلب","note":null,"examples":null},
      {"type":"item","it":"Cantante","ar":"مغني / مغنية","note":null,"examples":null},
      {"type":"item","it":"CD","ar":"سي دي","note":null,"examples":null},
      {"type":"item","it":"Chiave","ar":"مفتاح","note":null,"examples":null},
      {"type":"item","it":"Città","ar":"مدينة","note":null,"examples":null},
      {"type":"item","it":"Esame","ar":"امتحان","note":null,"examples":null},
      {"type":"item","it":"Esercizio","ar":"تمرين","note":null,"examples":null},
      {"type":"item","it":"Forma","ar":"شكل","note":null,"examples":null},
      {"type":"item","it":"Fratello","ar":"أخ","note":null,"examples":null},
      {"type":"item","it":"Fumetto","ar":"مجلة كوميكس","note":null,"examples":null},
      {"type":"item","it":"Insegnante","ar":"مدرّس / مدرّسة","note":null,"examples":null},
      {"type":"item","it":"Lezione","ar":"حصة / درس","note":null,"examples":null},
      {"type":"item","it":"Libro","ar":"كتاب","note":null,"examples":null},
      {"type":"item","it":"Macchina","ar":"عربية","note":null,"examples":null},
      {"type":"item","it":"Materia","ar":"مادة (دراسية)","note":null,"examples":null},
      {"type":"item","it":"Mese","ar":"شهر","note":null,"examples":null},
      {"type":"item","it":"Musica","ar":"موسيقى","note":null,"examples":null},
      {"type":"item","it":"Padre","ar":"أب","note":null,"examples":null},
      {"type":"item","it":"Pagina","ar":"صفحة","note":null,"examples":null},
      {"type":"item","it":"Palazzo","ar":"عمارة / قصر","note":null,"examples":null},
      {"type":"item","it":"Panino","ar":"ساندوتش","note":null,"examples":null},
      {"type":"item","it":"Piatto","ar":"طبق","note":null,"examples":null},
      {"type":"item","it":"Professore","ar":"أستاذ","note":null,"examples":null},
      {"type":"item","it":"Ragazzo","ar":"شاب / ولد","note":null,"examples":null},
      {"type":"item","it":"SMS","ar":"رسالة نصية","note":null,"examples":null},
      {"type":"item","it":"Sorella","ar":"أخت","note":null,"examples":null},
      {"type":"item","it":"Spaghetti","ar":"مكرونة سباجيتي","note":null,"examples":null},
      {"type":"item","it":"Sport","ar":"رياضة","note":null,"examples":null},
      {"type":"item","it":"Studio","ar":"مكتب / استوديو (دراسة)","note":null,"examples":null},
      {"type":"item","it":"Zaino","ar":"شنطة ضهر","note":null,"examples":null},
      {"type":"item","it":"Zio","ar":"عم / خال","note":null,"examples":null},
      {"type":"item","it":"Zona","ar":"منطقة","note":null,"examples":null},
      {"type":"item","it":"Zoo","ar":"حديقة حيوان","note":null,"examples":null},
      {"type":"item","it":"Donna","ar":"ست / امرأة","note":null,"examples":null},
      {"type":"item","it":"Calcio","ar":"كورة القدم","note":null,"examples":null},
      {"type":"item","it":"Amico","ar":"صاحب / صديق","note":"مؤنثها Amica","examples":null}
    ]
  },
  {
    "id": "lex_verbi",
    ...
    "blocks": [
      ...
      {"type":"item","it":"Sentire","ar":"يسمع / يحس","note":"مصدر","examples":null,"formAliases":["senta"]},
      ...
      {"type":"item","it":"Piace","ar":"بيعجبه / عاجبها","note":"Piacere، Presente — بيتعدى بضمير غير مباشر: mi piace, le piace...","examples":[{"it":"Non le piace questa situazione","ar":"الوضع ده مش عاجبها"}],"formAliases":["piacciono","piacciano"]},
      {"type":"item","it":"Sembrare","ar":"يبان / يظهر إنه","note":"نفس نظام Piacere بالظبط (ضمير جر غير مباشر + الفعل) — piace/sembra للمفرد، piacciono/sembrano للجمع","examples":[{"it":"Mi sembra un po' troppo giovanile","ar":"حاسة إنه شبابي أكتر من اللازم شوية"}],"formAliases":["sembra","sembrano"]},
      ...
      {"type":"item","it":"Portare","ar":"يلبس (وكمان: يحمل)","note":"مصدر","examples":[{"it":"Porta anche una cravatta","ar":"لابس كمان كرافتة"}],"formAliases":["porta"]},
      {"type":"item","it":"Mettere","ar":"يلبس (وكمان: يحط)","note":"مصدر","examples":[{"it":"Mette sempre i jeans","ar":"دايمًا بيلبس جينز"}],"formAliases":["mette"]},
      {"type":"item","it":"Amare","ar":"يحب","note":"مصدر","examples":[{"it":"Ama l'abbigliamento classico","ar":"بتحب اللبس الكلاسيك"}],"formAliases":["ama"]},
      {"type":"item","it":"Desiderare","ar":"يحتاج / يرغب","note":"مصدر — بصيغة السؤال Desidera؟ بتبقى تعبير أدب في المحل (تحت أمرك؟)","examples":null,"formAliases":["desidera"]},
      {"type":"item","it":"Cercare","ar":"يدوّر على","note":"مصدر","examples":[{"it":"Cerco un pullover","ar":"بدوّر على بلوفر"}],"formAliases":["cerco"]},
      {"type":"item","it":"Sapere","ar":"يعرف","note":"مصدر","examples":null,"formAliases":["sa","so"]},
      {"type":"item","it":"Costare","ar":"يتكلف","note":"مصدر","examples":null,"formAliases":["costa"]},
      {"type":"item","it":"Conservare","ar":"يحتفظ بـ","note":"مصدر","examples":[{"it":"Deve conservare lo scontrino","ar":"لازم تحتفظي بالإيصال"}]},
      {"type":"item","it":"Abitare","ar":"يسكن","note":"مصدر","examples":null,"formAliases":["abito"]},
      {"type":"item","it":"Ascoltare","ar":"يسمع (بانتباه)","note":"مصدر","examples":null,"formAliases":["ascolto"]},
      {"type":"item","it":"Comprare","ar":"يشتري","note":"مصدر","examples":null,"formAliases":["compra"]},
      {"type":"item","it":"Guardare","ar":"يتفرّج","note":"مصدر","examples":null,"formAliases":["guardo"]},
      {"type":"item","it":"Leggere","ar":"يقرا","note":"مصدر","examples":null,"formAliases":["legge"]},
      {"type":"item","it":"Mandare","ar":"يبعت","note":"مصدر","examples":null,"formAliases":["manda"]},
      {"type":"item","it":"Scrivere","ar":"يكتب","note":"مصدر","examples":null,"formAliases":["scrive"]},
      {"type":"item","it":"Andare","ar":"يروح","note":"مصدر — فعل شاذ، لسه مش موجود بجدول تصريف كامل في تاب الأفعال","examples":null,"formAliases":["vado"]},
      {"type":"item","it":"Vivere","ar":"يعيش","note":"مصدر","examples":null,"formAliases":["vivo"]}
    ]
  },
  {
    "id": "lex_avverbi",
    ...
    "blocks": [
      ...
      {"type":"item","it":"Oggi","ar":"النهارده","note":null,"examples":null},
      ...
      {"type":"item","it":"Piuttosto","ar":"بدلاً من ذلك / بالأحرى","note":"بتُستخدم لاقتراح بديل بدل حاجة اتقالت أو متوقعة","examples":[{"it":"Dammi piuttosto il numero del portiere","ar":"أعطني بدلاً من ذلك رقم الحارس"}]},
      {"type":"item","it":"Soprattutto","ar":"خاصةً / وبالأخص","note":"بتتحط غالباً في آخر الجملة أو بعد فاصلة لتوضيح أهم نقطة","examples":[{"it":"Ce l'ho da tre giorni, soprattutto la sera","ar":"هو عندي من ثلاثة أيام، خاصةً في المساء"}]}
    ]
  },
  {
    "id": "prep_di",
  ...
  },
  {
    ...
    "blocks": [
      ...
      },
      {
        ...
        "form": "a",
        "examples": [
            {"it": "Sono favorevole all'apertura domenicale", "ar": "أنا موافق على الفتح يوم الحد", "form": "all'", "words":[{"it":"Sono","ar":"","note":"Essere، Presente","type":"verbo"},{"it":"favorevole","ar":"موافق","note":null,"type":"altro"},{"it":"all'","ar":"","type":"preposizione","grammarId":"prep_a","note":null},{"it":"apertura","ar":"فتح","note":null,"type":"altro"},{"it":"domenicale","ar":"يوم الحد (أسبوعي)","note":null,"type":"altro"}]},
  ...
  },
  {
    ...
    "blocks": [
      ...
      {"type":"item","it":"Collo","ar":"رقبة / ياقة","note":"في تعبير a collo alto = ياقة عالية (Alto هنا = عالي/طويل)","examples":null,"formAliases":["alto"]},
      ...
      {"type":"item","it":"Pullover","ar":"بلوفر","note":"مرادف لـ Maglione","examples":[{"it":"Cerco un pullover","ar":"بدوّر على بلوفر"}]},
      {"type":"item","it":"Capo","ar":"قطعة (لبس)","note":"اسم عام لأي قطعة لبس واحدة، غير Abbigliamento اللي بيجمع كل اللبس","examples":[{"it":"Deve conservare lo scontrino per questo capo","ar":"لازم تحتفظي بالإيصال بتاع القطعة دي"}]},
      {"type":"item","it":"Guanto","ar":"جونتي / قفاز","note":"جمعها Guanti","examples":[{"it":"I guanti di pelle","ar":"الجونتي الجلد"}],"formAliases":["guanti"]},
      {"type":"item","it":"Pelliccia","ar":"فرو","note":"جمعها Pellicce","examples":[{"it":"Le pellicce","ar":"الفراوي"}],"formAliases":["pellicce"]},
      {"type":"item","it":"Cappello","ar":"قبعة","note":"جمعها Cappelli","examples":[{"it":"I cappelli","ar":"القبعات"}],"formAliases":["cappelli"]},
      {"type":"item","it":"Sandalo","ar":"صندل","note":"جمعها Sandali","examples":[{"it":"I sandali","ar":"الصنادل"}],"formAliases":["sandali"]},
      {"type":"item","it":"Cintura","ar":"حزام","note":"جمعها Cinture","examples":[{"it":"Le cinture","ar":"الأحزمة"}],"formAliases":["cinture"]},
      {"type":"item","it":"Sciarpa","ar":"شال / سكارف","note":"جمعها Sciarpe","examples":[{"it":"Le sciarpe di lana","ar":"الشالات الصوف"}],"formAliases":["sciarpe"]}
    ]
  }
];
